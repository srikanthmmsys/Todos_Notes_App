...
const InputDataComponent = () => {
  ...
  const [todos,setTodos,addTodoHandeller,edit,setEdit,editmode,setEditMode]=useContext(context);
...
const formSubmitHandeller=(e)=>{
  ...
  if(!editmode){
      ...
      let totalData={headding:indata,description:desc,dt:fullDate,stared:false,deleted:false}
  ...
  return (<>
    ...
    <form id='inputDataComp' className='text-center container d-flex flex-column' onSubmit={formSubmitHandeller}>
      ...
      <label className='text-info'>Headding</label>
        ...
        <textarea className='m-2'rows={10} cols={24} value={desc} onChange={descHandeller} />
        </div>
        <div className='d-flex flex-row justify-content-center m-2 '>
          <div className='ml'></div>
          <Link to='/'><button className='m-2 btn btn-danger'>Cancel</button></Link>
          <button className='m-2 btn btn-success' type='submit'>Submit</button>
        </div>  
    </form>
